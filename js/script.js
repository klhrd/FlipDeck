...
// ===== 應用程式腳本 (v4.1.0 Mastery System - Stabilized) =====
document.addEventListener('DOMContentLoaded', () => {
    ...
    let upcomingQueue = []; // v4.1.0 新增：穩定堆疊隊列
    ...
    function loadGlobalPreferences() {
        ...
        globalPreferences = saved ? JSON.parse(saved) : {};
    }
    ...
    function startGame(deckId) {
        ...
        if (!deckData || !deckMeta) { showError("載入失敗"); renderHomeScreen(); return; }
        ...
        currentFullDeck.forEach(card => {
            if (!currentCardStatus.has(card.card_id) || typeof currentCardStatus.get(card.card_id) !== 'object') {
                currentCardStatus.set(card.card_id, { mastery: 1, lastTime: Date.now() });
            }
        });
    ...
    function updateLearningDeck() {
        ...
        if (currentDeckSettings.filterTags?.length > 0) {
            filtered = currentFullDeck.filter(card => {
                ...
                return currentDeckSettings.filterTags.some(t => t === 'untagged' ? tags.length === 0 : tags.includes(t));
            });
        }
    ...
    function fillUpcomingQueue() {
        while (upcomingQueue.length < 5 && currentLearningDeck.length > 0) {
            ...
            const card = currentLearningDeck[pickIdx];
            if (!upcomingQueue.includes(card.card_id)) {
                upcomingQueue.push(card.card_id);
            } else if (currentLearningDeck.length <= upcomingQueue.length) {
                break; // 沒得挑了
            }
        }
    }
    ...
    function showPrevCard() {
        if (historyIndex > 0) { historyIndex--; displayCardStack(); }
        else { historyAlert.classList.remove('hidden'); setTimeout(() => historyAlert.classList.add('hidden'), 1500); }
        updateUI();
    }
    ...
    function displayCardStack() {
        ...
        if (historyIndex === history.length - 1) {
            for (let i = 0; i < 2 && upcomingQueue[i]; i++) {
                visible.push(currentFullDeck.find(c => c.card_id === upcomingQueue[i]));
            }
        } else {
        ...
        for (let i = visible.length - 1; i >= 0; i--) {
            ...
            if (isTop) {
                ...
                cardEl.innerHTML = `
                    ...
                    <div class="card-inner">
                        <div class="absolute inset-0 backface-hidden bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl shadow-sm flex items-center justify-center p-8">
                            <div class="card-content text-center text-neutral-900 dark:text-neutral-100 font-bold" style="font-size: ${currentDeckSettings.fontSize}rem">${cardData.fields[0]}</div>
                        </div>
                        <div class="absolute inset-0 backface-hidden rotate-y-180 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 rounded-3xl shadow-lg flex flex-col items-center justify-center p-8 text-center">
                            <div class="card-content font-bold mb-4" style="font-size: ${currentDeckSettings.fontSize * 0.8}rem">${cardData.fields[1] || ''}</div>
                ...
                cardEl.innerHTML = `<div class="w-full h-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl flex items-center justify-center p-8"><div class="text-neutral-200 dark:text-neutral-800 font-bold truncate w-full text-center" style="font-size: ${currentDeckSettings.fontSize}rem">${cardData.fields[0]}</div></div>`;
                cardStackContainer.appendChild(cardEl);
            }
        }
    }
    ...
    function initComboLogic(card) {
        ...
        const sForgot = card.querySelector('#stamp-forgot');
        const sLearned = card.querySelector('#stamp-learned');
        ...
        card.addEventListener('pointerdown', (e) => {
            ...
            isDragging = true; startX = lastX = e.clientX; lastTime = startTime = Date.now(); dragX = 0;
        ...
        swipeMoveHandler = (e) => {
            ...
            if (dragX > 0) {
                sLearned.style.opacity = opacity; sForgot.style.opacity = 0;
                bgLearned.style.opacity = opacity; bgForgot.style.opacity = 0;
            } else {
                sForgot.style.opacity = opacity; sLearned.style.opacity = 0;
                bgForgot.style.opacity = opacity; bgLearned.style.opacity = 0;
            }
        };
    ...
    function displayEndOfDeck() {
        cardStackContainer.innerHTML = `
            <div class="flex flex-col items-center justify-center text-center p-8 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl shadow-lg animate-in zoom-in duration-300">
                <div class="w-20 h-20 bg-green-50 dark:bg-green-900/20 text-green-500 rounded-full flex items-center justify-center mb-6"><span class="material-symbols-outlined !text-5xl">celebration</span></div>
                <h2 class="text-3xl font-bold mb-4 text-neutral-900 dark:text-neutral-100">複習完成！</h2>
                <button id="restart-btn" class="px-8 py-3 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 font-bold rounded-full shadow-md hover:scale-105 transition-all">重新開始</button>
            </div>
        `;
    ...
    function bindEventListeners() {
        // 使用 stopPropagation 避免冒泡導致選單關閉或按鈕失效
        ...
        [homeMenuFullscreenBtn, deckMenuFullscreenBtn].forEach(btn => btn?.addEventListener('click', (e) => { e.stopPropagation(); toggleFullScreen(); }));
        [homeMenuAddBtn, deckMenuAddBtn].forEach(btn => btn?.addEventListener('click', (e) => { e.stopPropagation(); openQuickStartModal(); }));
        [homeMenuHomeBtn, deckMenuHomeBtn].forEach(btn => btn?.addEventListener('click', (e) => { e.stopPropagation(); backToHomeScreen(); hideAllMenus(); }));
        
        homeMenuCloseBtn.addEventListener('click', hideAllMenus); 
        ...
        deckMenuCloseBtn.addEventListener('click', hideAllMenus); 
    ...
    function handleFileUpload(e) {
        ...
        reader.onload = (ev) => {
            try {
                const data = file.name.endsWith('.json') ? JSON.parse(ev.target.result) : ev.target.result.trim().split('\n').map((l, i) => ({ card_id: `card_${i}_${crypto.randomUUID()}`, fields: l.split(';').map(f => f.trim().replace(/\/\//g, '<br>')), tags: [], notes: "" }));
    ...
    function handleExampleLoad() {
        const file = exampleSelector.value; if (!file) return;
    ...
    function processNewDeck(deckData, deckId, title, source) {
        ...
        const meta = { id: deckId, title, totalCards: deckData.length, lastOpened: Date.now(), settings: { ...DEFAULT_DECK_SETTINGS } };
        const idx = deckList.findIndex(d => d.id === deckId);
        if (idx >= 0) deckList[idx] = meta; else deckList.unshift(meta);
        saveDeckList(); resetDeckState(deckId, deckData); startGame(deckId);
    }
    ...
    function renderHomeScreen() {
        ...
        sorted.forEach(deck => {
            ...
            card.innerHTML = `
                <div class="absolute top-0 left-0 w-1.5 h-full bg-teal-500 opacity-0 group-hover:opacity-100 transition-opacity"></div>
        ...
        statsTotalDecks.textContent = deckList.length; statsTotalCards.textContent = totalCards;
    }
    ...
    function populateCardTagFilters() {
        ...
        cardTagFilterContainer.appendChild(createTagFilterCheckbox("未分類", "untagged"));
    ...
    function openEditDeckModal() {
        ...
        editDeckModal.classList.remove('hidden'); hideAllMenus();
    }
    ...
    function saveDeckEdit() {
        try {
            ...
            saveDeckList(); startGame(currentDeckId); editDeckModal.classList.add('hidden');
    ...
    function addNewTag() {
        ...
        if (tag && !allTags.includes(tag)) {
            allTags.push(tag); saveAllTags(); newTagInput.value = ''; renderTagList();
        }
    }
    ...
    function backupAllData() {
        ...
        zip.generateAsync({type:"blob"}).then(content => {
            ...
            a.download = `FlipDeck_Backup.zip`; a.click();
        });
    }
    ...
    function openDeckEditor(deckId = null) {
        ...
        if (deckId) {
            ...
            editorFieldCount.value = meta.fieldCount || 2;
    ...
    function saveDeckFromEditor() {
        const title = editorDeckTitle.value.trim(); if (!title) return;
        const cards = [];
    ...
    function downloadData(format) {
        ...
        const a = document.createElement("a");
        a.href = URL.createObjectURL(b); a.download = `Deck.${ext}`; a.click();
    }
